...
func doEncrypt(cmd *cobra.Command, args []string) error {
	...
	if encryptOpts.secretFile == "" {
		...
		if err := flushToFile(secretMap, encryptOpts.secretFile, encryptOpts.kmsKey); err != nil {
			return errors.Wrapf(err, "Failed to flush secrets to file %s", encryptOpts.secretFile)
		}
	}
...
func flushToFile(secretMap map[string]string, filename, kmsKey string) error {
	...
	if _, err := os.Stat(filename); err == nil {
		...
		if err2 != nil {
			return errors.Wrapf(err2, "Failed to load local secret file %s", filename)
		}

		if kmsKey != currentKmsKey {
			return errors.Errorf("KMS key alias %s does not match to %s", kmsKey, currentKmsKey)
		}
	...
	if err := newSecrets.SaveAsYAML(filename, kmsKey); err != nil {
		return errors.Wrapf(err, "Failed to update local secret file %s", filename)
	}
...
func readFromStdin(kmsKey string) (map[string]string, error) {
	...
	for _, line := range lines {
		...
		if err != nil {
			return map[string]string{}, errors.Wrapf(err, "Failed to encrypt secret %q", key)
		}
...
func readFromArgs(args []string, kmsKey string) (map[string]string, error) {
	...
	for _, arg := range args {
		...
		if len(ss) < 2 {
			return map[string]string{}, errors.Errorf("Given argument %q is invalid format, should be KEY=VALUE", arg)
		}
		key, value := ss[0], ss[1]
